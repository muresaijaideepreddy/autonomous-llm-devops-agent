...
def generate_tests_with_gemini(
    ...
    Supports coverage-aware generation when coverage_context is provided.
    """

    max_tests = MAX_TESTS_BY_SEVERITY.get(risk_level, 2)

    # 🔑 Coverage-aware instruction (ONLY when healer triggers)
    coverage_instruction = ""
    if coverage_context:
        coverage_instruction = f"""
IMPORTANT (COVERAGE HEALING MODE):
The following lines are NOT covered by tests:
{coverage_context}

Rules:
- Generate tests ONLY to execute these uncovered lines
- Do NOT re-test happy paths
- Do NOT change business logic
- If uncovered code raises errors, ASSERT the error
- Do NOT assert incorrect behavior
"""
...
def tester_agent(plan: Dict) -> Dict:
    ...
    Input:
        plan:
            ...
            risk_level: str
            coverage_context: Optional[dict]   # <-- ONLY from Coverage Healer
